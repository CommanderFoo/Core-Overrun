...
function Database:GetItemFromName(itemName)
    ...
    assert(item,string.format("Failed to create item - %s from database. Are you sure this is the right name?",itemName))
    return item
end
...
function Database:GetItemFromMUID(muid)
    ...
    assert(itemData,string.format("Failed to create item - %s from database. Are you sure this is the right muid?",muid))
    local item = Item.New(itemData)
    return item
end
...
function Database:GetItemFromIndex(index)
    ...
    local item = Item.new(itemData)
    assert(item,string.format("Failed to create item - %s from database. Are you sure this is the right index?",index))
    return item
end

function Database:CreateItemFromHash(itemHash)
    local item = Item.FromHash(self, itemHash)
    assert(item,string.format("Failed to create item - %s from database. This may not be a valid hash.",itemHash))
    return item
end
...
function Database:_LoadDrops()
    ...
    for _, lootTable in pairs(LOOT_TABLE) do
        ...
        for _, lootDrop in pairs(lootTable:GetChildren()) do
            local item = lootDrop:GetCustomProperty("Item"):GetObject() or "Empty"
    ...
    if dropTable then
        ...
        for _,dropInfo in ipairs(dropTable) do
            if roll <= dropInfo.likelihood then
                return self:FindItemDataByName(dropInfo.itemName), dropInfo.min, dropInfo.max 
            end
