...
local_player.bindingPressedEvent:Connect(function(player, binding)
	...
	elseif(binding == "ability_extra_20") then
		...
		if(stats.visibility == Visibility.FORCE_ON) then
			...
			total_kills.text = YOOTIL.Utils.number_format(local_player:GetResource("total_kills"))
			...
			highest_round.text = YOOTIL.Utils.number_format(local_player:GetResource("highest_round"))
